...
const autoresponse = {
    ...
    async execute(message) {
        ...
        if (!afkUsers) return;
        afkUsers.forEach(u => {
            if (mentionned.has(u.user_id)) message.channel.send(`**${mentionned.get(u.user_id).username}** est AFK depuis ${timeToString(Math.floor((Date.now() - Date.parse(u.time)) / 1000))}${u.message ? ` : **${u.message}**` : ""} 💤`).catch(console.error);
