...
import { formatName, FormatType, pokemonImage, Variation, VariationType } from "../../utils/pokemon/pokemonInfo";

import { Collection } from "discord.js";

export default class Pokedex {
	static pokemons: Pokemon[] = require("../../assets/pokemons.json").map((rawPokemon: RawPokemon) => {
		const pokemon: Pokemon = {
			...
			heightUs: `${Math.floor(rawPokemon.height * 3.281)}'${Math.round((rawPokemon.height * 3.281) % 12)
				.toString()
				.padStart(2, "0")}"`,
			...
			megaEvolutions: rawPokemon.mega_evolutions.map((megaEvolution): MegaEvolution => {
				return {
					...
					heightUs: `${Math.floor(megaEvolution.height * 3.281)}'${Math.round((rawPokemon.height * 3.281) % 12)
						.toString()
						.padStart(2, "0")}"`,
			...
			variations: rawPokemon.variations.map((rawPokemonVariation): PokemonVariation => {
				return {
			...
			formatName: (
				...
				variation?: Variation,
				format?: FormatType,
			) => formatName(this.findById(rawPokemon.national_id), shiny, variationType, variation, language, format),
			image: (shiny?: boolean, variationType?: VariationType, variation?: Variation) =>
				pokemonImage(this.findById(rawPokemon.national_id), shiny, variationType, variation),
		};
	...
	static megaEvolvablePokemons: Collection<number, Pokemon> = new Collection(
		this.pokemons.filter((pokemon) => pokemon.megaEvolutions.length).map((pokemon) => [pokemon.nationalId, pokemon]),
	);

	static alolaPokemons: Collection<number, Pokemon> = new Collection(
		this.pokemons
			.filter((pokemon) => pokemon.variations.some((variation) => variation.variationType === "alola"))
			.map((pokemon) => [pokemon.nationalId, pokemon]),
	);

	static galarPokemons: Collection<number, Pokemon> = new Collection(
		this.pokemons
			.filter((pokemon) => pokemon.variations.some((variation) => variation.variationType === "galar"))
			.map((pokemon) => [pokemon.nationalId, pokemon]),
	);
		...
		if (!pokemon) {
			pokemon = this.pokemons.find((pkm) =>
				pkm.megaEvolutions.some((megaEvolution) =>
					Object.values(megaEvolution.names).some((n) => n.toLowerCase() === name?.toLowerCase()),
				),
			);
		...
		if (!pokemon) {
			pokemon = this.pokemons.find((pkm) =>
				pkm.variations.some((variation) =>
					Object.values(variation.names).some((n) => n.toLowerCase() === name?.toLowerCase()),
				),
			);
	...
	static defaultCatchRates = this.pokemons.map(
		(pokemon, i, dex) => pokemon.catchRate + dex.slice(0, i).reduce((sum, p) => sum + p.catchRate, 0),
	);
}
