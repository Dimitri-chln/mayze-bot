...
export default class CatchRates {
	...
	constructor(pokemonList: PokemonList, upgrades: DatabaseUpgrades) {
		...
		for (const pokemon of Util.pokedex.pokemons) {
			...
			if (pokemon.legendary || pokemon.ultraBeast) probability *= 1 + (upgrades.legendary_ub_probability * 2) / 100;
			if (!pokemonList.has(pokemon)) probability *= 1 + (upgrades.new_pokemon_probability * 2) / 100;
