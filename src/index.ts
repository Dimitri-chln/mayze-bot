...
setInterval(
	() => reconnectDatabase(Util.database),
	3600000
);



const directories = Fs.readdirSync(Path.resolve(__dirname, "commands"), { withFileTypes: true })
	.filter(dirent => dirent.isDirectory() && dirent.name !== "disabled")
	.map(dirent => dirent.name);

for (const directory of directories) {
	const commandFiles = Fs.readdirSync(Path.resolve(__dirname, "commands", directory))
...
for (const commandName of Util.commands.keys()) {
	Util.commandCooldowns.set(commandName, new Discord.Collection());
}
...
client.on("ready", async () => {
	...
	// Slash commands
	await Promise.all(Util.commands.map(async command => {
		...
		const applicationCommand = client.application.commands.cache.find(cmd => cmd.name === applicationCommandData.name);
		...
		if (command.category === "admin") {
			try {
				...
				if (applicationCommand && !applicationCommand.equals(applicationCommandData)) {
					...
					newApplicationCommand = await client.application.commands.edit(applicationCommand.id, applicationCommandData, Util.config.ADMIN_GUILD_ID);
				
				} else {
					console.log(`Creating the admin application command /${applicationCommandData.name}`);
					newApplicationCommand = await client.application.commands.create(applicationCommandData, Util.config.ADMIN_GUILD_ID);
				}
		...
		if (command.guildIds) {
			if (applicationCommand && !applicationCommand.equals(applicationCommandData)) {
				...
				for (const guildId of command.guildIds) {
					applicationCommandData.description = command.description[Util.languages.get(guildId)];
					applicationCommandData.options = command.options[Util.languages.get(guildId)];
	...
	(roseChannel as Discord.TextChannel).messages.fetch({ limit: 1 })
		.then(([ [ , logMessage ] ]) => {
			...
			Util.roseLobby = new Cron.CronJob(date, () => {
				(announcementChannel as Discord.TextChannel).send(`<@&${Util.config.ROSE_LOBBY_ROLE_ID}>\nLa game de roses va démarrer, le mot de passe est \`${password}\``).catch(console.error);
	...
	setInterval(async () => {
		try {
			const { rows: reminders }: { rows: DatabaseReminder[] } = await Util.database.query("SELECT * FROM reminders");
	...
	switch (interaction.type) {
		...
		case "APPLICATION_COMMAND": {
			if (interaction.isCommand()) {
				...
				if (Util.channelCooldowns.get(interaction.channel.id)) return;
				if (command) runApplicationCommand(command, interaction).catch(console.error);
			}
...
client.on("messageCreate", async message => {
	...
	if (
		...
		if (bots) {
			...
			if (!prefixes.some(p => message.content.toLowerCase().startsWith(p))) {
				...
				const newXP = Math.round(Math.sqrt(message.content.length) * Util.config.XP_MULTIPLIER / Util.xpMessages.get(message.author.id));
	...
	// Message responses
	Util.messageResponses.forEach(async messageResponse => messageResponse.run(message).catch(console.error));
})
...
client.on("messageDelete", async message => {
	...
	Util.sniping.deletedMessages.set(message.channel.id, message as Discord.Message);
...
client.on("messageUpdate", async (message, newMessage) => {
	...
	Util.sniping.editedMessages.set(message.channel.id, message as Discord.Message);
...
client.on("messageReactionAdd", async (reaction, user) => {
	...
	for (const reactionCommand of Util.reactionCommands) {
		reactionCommand.run(reaction as Discord.MessageReaction, user as Discord.User, true)
			.catch(console.error);
	}
})
...
client.on("messageReactionRemove", async (reaction, user) => {
	...
	for (const reactionCommand of Util.reactionCommands) {
		reactionCommand.run(reaction as Discord.MessageReaction, user as Discord.User, false)
			.catch(console.error);
	}
...
client.on("guildMemberAdd", async member => {
	...
	try {
		const { rows: [ memberRoles ] } = await Util.database.query(
	...
	member.roles.add(roleIds.map(roleId => member.guild.roles.cache.get(roleId))).catch(console.error);
})
...
function testReminders() {
	...
	const wednesday = new Cron.CronJob("0 45 15 * * 3", () => {
		userIds.forEach(userId => client.users.fetch(userId).then(u => u.send(getMessage(15)).catch(console.error)));
		setTimeout(() => userIds.forEach(userId => client.users.cache.get(userId).send(getMessage(5)).catch(console.error)), 600000);
		setTimeout(() => userIds.forEach(userId => client.users.cache.get(userId).send(getMessage(2)).catch(console.error)), 780000);
	}, null, true, null, null, false, 0);

	const sunday = new Cron.CronJob("0 45 13 * * 0", () => {
		userIds.forEach(userId => client.users.fetch(userId).then(userId => userId.send(getMessage(15)).catch(console.error)));
		setTimeout(() => userIds.forEach(userId => client.users.cache.get(userId).send(getMessage(5)).catch(console.error)), 600000);
		setTimeout(() => userIds.forEach(userId => client.users.cache.get(userId).send(getMessage(2)).catch(console.error)), 780000);
	}, null, true, null, null, false, 0);
