...
const REGEX_LIST = {
	YOUTUBE_VIDEO:
		/^((?:https?:)\/\/)?((?:www|m)\.)?((?:youtube\.com|youtu.be))((?!channel)(?!user)\/(?:[\w\-]+\?v=|embed\/|v\/)?)((?!channel)(?!user)[\w\-]+)(\S+)?$/,
	YOUTUBE_VIDEO_ID:
	...
	SPOTIFY_PLAYLIST:
		/https?:\/\/(?:embed\.|open\.)(?:spotify\.com\/)(?:(album|playlist)\/|\?uri=spotify:playlist:)((\w|-){22})(?:(?=\?)(?:[?&]foo=(\d*)(?=[&#]|$)|(?![?&]foo=)[^#])+)?(?=#|$)/,
	DEEZER_SONG: /https?:\/\/(?:www\.)?deezer\.com\/(?:\w{2}\/)?track\/(\d+)/,
	DEEZER_PLAYLIST:
		/https?:\/\/(?:www\.)?deezer\.com\/(?:\w{2}\/)?(?:playlist|album)\/(\d+)/,
};
...
export default class MusicUtil {
	...
	static async search(
		...
		requestedBy: User,
		limit: number = 1,
		searchOptions: Partial<PlayOptions> = {},
	) {
		...
		if (searchOptions.uploadDate)
			filters =
				Array.from(
					...
					o.name.toLowerCase().includes(searchOptions.uploadDate),
				) ?? filters;
		...
		if (searchOptions.duration)
			filters =
				Array.from(
					...
					o.name.toLowerCase().startsWith(searchOptions.duration),
				) ?? filters;
		...
		if (searchOptions.sortBy)
			filters =
				Array.from(
					...
					o.name.toLowerCase().startsWith(searchOptions.sortBy),
				) ?? filters;

		try {
			const result = await ytsr(filters.url, { limit });
	...
	static async playlist(
		...
		if (isSpotifyPlaylistLink) {
			const spotifyResult = await Spotify.getData(search).catch(
				() => null,
			);
			if (
				!spotifyResult ||
				!["playlist", "album"].includes(spotifyResult.type)
			)
		...
		if (isDeezerPlaylistLink) {
			...
			const deezerResult = await deezer
				.playlist(playlistId)
				.catch(() => null);
			if (
				!deezerResult ||
				!["playlist", "album"].includes(deezerResult.type)
			)
				throw "InvalidPlaylist";
			...
			const playlistData: PlaylistData = {
				...
				channel:
					deezerResult.type === "playlist"
						? { name: deezerResult.owner.display_name }
						: deezerResult.artists[0],
				url: search,
		...
		if (isYouTubePlaylistLink) {
			...
			playlistData.videos = youtubeResult.videos
				.map((video, index) => {
					if (limit !== -1 && index >= limit) return;
					const songData: SongData = {
						title: video.title,
	...
	static deserializeOptionsPlay(
		options: Partial<PlayOptions> | string,
	): Partial<PlayOptions> {
	...
	static deserializeOptionsPlaylist(
		options: Partial<PlaylistOptions> | string,
	): Partial<PlaylistOptions> {
		if (options && typeof options === "object")
