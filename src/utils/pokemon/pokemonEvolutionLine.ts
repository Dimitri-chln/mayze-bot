...
function _evolutionLine(pokemon: Pokemon, origin = true): EvolutionLine {
	...
	return {
		...
		evolutions: pokemon
			.evolutions()
			.map((evolution) => _evolutionLine(evolution, false)),
	};
}

function _flatEvolutionLine(
	pokemon: Pokemon,
	origin?: EvolutionLine,
): Pokemon[] {
	...
	for (const evolution of evoLine.evolutions)
		flatEvoLine.push(..._flatEvolutionLine(evolution.pokemon, evolution));

	return flatEvoLine;
}

function _stringEvolutionLine(
	pokemon: Pokemon,
	language: Language,
	origin?: EvolutionLine,
	depth = 0,
	...
	let stringEvoLine = `${"\t".repeat(depth)}${
		evoLine.pokemon.names[language] ?? evoLine.pokemon.names.en
	}\n`;

	for (const evolution of evoLine.evolutions) {
		stringEvoLine += _stringEvolutionLine(
			evolution.pokemon,
			language,
			evolution,
			depth + 1,
