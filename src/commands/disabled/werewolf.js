	...
	run: async (message, args, options, language, languageCode) => {
		...
		switch (subCommand) {
			case "join": {
				...
				await message.member.roles.remove(unJailedRoles);
				await message.member.roles.add(jailedRoles);
			...
			case "leave": {
				...
				const jailedRoles = message.member.roles.cache.filter(role => message.guild.roles.cache.some(r => r.permissions.has("ADMINISTRATOR") && role.name === r.name + " (Jailed)"));
				const unJailedRoles = message.guild.roles.cache.filter(role => role.permissions.has("ADMINISTRATOR") && message.member.roles.cache.some(r => r.name === role.name + " (Jailed)"));
				jailedRoles.set(roleIngame.id, roleIngame);

				await message.member.roles.add(unJailedRoles).catch(console.error);
				await message.member.roles.remove(jailedRoles).catch(console.error);
