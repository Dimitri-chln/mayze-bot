			...
			case "pokemons": {
				...
				switch (subCommand) {
					case "global": {
						...
						const { rows: [ normal ] }: { rows: Omit<DatabasePokemonWithCaughtStats, keyof DatabasePokemon>[] } = await Util.database.query(
							`
						...
						description += translations.data.normal((normal.total ?? 0).toString());
						total += normal.total ?? 0;

						const { rows: [ shiny ] }: { rows: Omit<DatabasePokemonWithCaughtStats, keyof DatabasePokemon>[] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						description += translations.data.shiny((shiny.total ?? 0).toString());
						total += shiny.total ?? 0;

						const { rows: allNormal }: { rows: DatabasePokemon[] } = await Util.database.query(
							`
						...
						const legendary = allNormal.filter(pokemon => Pokedex.findById(pokemon.pokedex_id).legendary);
						const legendaryTotal = legendary.reduce(
							(total, pkm) => total + Object.values(pkm.users).reduce(
								(sum, user) => sum + user.caught, 0
							),
							0
						);
						...
						const ultraBeast = allNormal.filter(pokemon => Pokedex.findById(pokemon.pokedex_id).ultraBeast);
						...
						const { rows: allShiny }: { rows: DatabasePokemon[] } = await Util.database.query(
							`
							SELECT pokedex_id, users
							FROM pokemons
							WHERE shiny AND variation = 'default'
						...
						const legendaryShiny = allShiny.filter(pokemon => Pokedex.findById(pokemon.pokedex_id).legendary);
						...
						const ultraBeastShiny = allShiny.filter(pokemon => Pokedex.findById(pokemon.pokedex_id).ultraBeast);
						...
						const { rows: [ alola ] }: { rows: Omit<DatabasePokemonWithCaughtStats, keyof DatabasePokemon>[] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						description += translations.data.alola((alola.total ?? 0).toString());
						total += alola.total ?? 0;

						const { rows: [ alolaShiny ] }: { rows: Omit<DatabasePokemonWithCaughtStats, keyof DatabasePokemon>[] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						description += translations.data.alola_shiny((alolaShiny.total ?? 0).toString());
						total += alolaShiny.total ?? 0;

						const { rows: [ mega ] }: { rows: Omit<DatabasePokemonWithCaughtStats, keyof DatabasePokemon>[] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						description += translations.data.mega((mega.total ?? 0).toString());
						total += mega.total ?? 0;

						const { rows: [ megaShiny ] }: { rows: Omit<DatabasePokemonWithCaughtStats, keyof DatabasePokemon>[] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						description += translations.data.mega_shiny((megaShiny.total ?? 0).toString());
					...
					case "about": {
						...
						const { rows: [ normal ] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						const { rows: [ shiny ] } = await Util.database.query(
							`
							SELECT SUM((value -> 'caught')::int) AS total
							FROM pokemons, jsonb_each(users)
						...
						if (pokemon.variations.some(variation => variation.suffix === "alola")) {
							const { rows: [ alola ] } = await Util.database.query(
								`
								SELECT SUM((value -> 'caught')::int) AS total
								FROM pokemons, jsonb_each(users)
							...
							const { rows: [ alolaShiny ] } = await Util.database.query(
								`
								SELECT SUM((value -> 'caught')::int) AS total
								FROM pokemons, jsonb_each(users)
						...
						if (pokemon.megaEvolutions.length) {
							const { rows: [ mega ] } = await Util.database.query(
								`
								SELECT SUM((value -> 'caught')::int) AS total
								FROM pokemons, jsonb_each(users)
							...
							const { rows: [ megaShiny ] } = await Util.database.query(
								`
								SELECT SUM((value -> 'caught')::int) AS total
								FROM pokemons, jsonb_each(users)
								WHERE pokedex_id = $1 AND shiny = false AND variation = ANY('{ "mega", "megax", "megay", "primal" }')
					...
					case "caught": {
						...
						const variation: VariationType = interaction.options.getString("variation") as "mega" | "alola" ?? "default";

						let { rows: pokemons }: { rows: DatabasePokemonWithCaughtStats[] } = await Util.database.query(
							`
						...
						pokemons = pokemons.filter(pokemon => {
							if (interaction.options.getBoolean("legendary") && !Pokedex.findById(pokemon.pokedex_id).legendary) return false;
							if (interaction.options.getBoolean("ultra-beast") && !Pokedex.findById(pokemon.pokedex_id).ultraBeast) return false;
						...
						for (let i = 0; i < pokemons.length; i += Util.config.ITEMS_PER_PAGE) {
							page.embeds[0].description = pokemons.slice(i, i + Util.config.ITEMS_PER_PAGE).map((pokemon, j) =>
								translations.data.most_caught_description(
									(i + j + 1).toString(),
									Pokedex.findById(pokemon.pokedex_id).formatName(shiny, variation, translations.language),
