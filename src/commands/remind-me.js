...
const command = {
	...
	*/
	run: async (message, args, options, language, languageCode) => {
		...
		const { "rows": reminders } = (await message.client.database.query(`SELECT * FROM reminders WHERE user_id = '${message.author.id}'`).catch(console.error)) || {};
		...
		switch (subCommand) {
			case "create": {
				...
				const res = await message.client.database.query(`INSERT INTO reminders (user_id, timestamp, content) VALUES ('${message.author.id}', '${timestamp}', '${content.replace(/'/g, "''")}')`).catch(console.error);
			...
			case "remove": {
				...
				const res = await message.client.database.query(`DELETE FROM reminders WHERE id = ${reminders[index - 1].id}`).catch(console.error);
				if (!res) return message.channel.send(language.errors.database).catch(console.error);
