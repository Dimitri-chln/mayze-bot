...
const command = {
	...
	*/
	run: async (message, args, options, language, languageCode) => {
		...
		const { "rows": notes } = (await message.client.database.query(`SELECT * FROM mudae_notes WHERE user_id = '${message.author.id}'`).catch(console.error)) || {};
		if (!notes) return message.channel.send(language.errors.database).catch(console.error);
		
		switch(subCommand) {
			case "add": {
				const res = await message.client.database.query(`INSERT INTO mudae_notes (user_id, note) VALUES ('${message.author.id}', '${note}')`).catch(console.error);
			...
			case "remove": {
				...
				const res = await message.client.database.query(`DELETE FROM mudae_notes WHERE user_id = '${message.author.id}' AND note = '${noteToRemove.note}'`).catch(console.error);
				if (!res) return message.channel.send(language.errors.database).catch(console.error);
				if (!message.isInteraction) message.react("✅").catch(console.error);
