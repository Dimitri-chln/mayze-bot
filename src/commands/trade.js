	...
	run: async (message, args, options, language, languageCode) => {
		...
		switch (subCommand) {
			case "block":
				...
				const res = (await message.client.database.query(`SELECT * FROM trade_block WHERE user_id = '${message.author.id}' AND blocked_user_id = '${user.id}'`).catch(console.error));
				if (!res) return message.channel.send(language.errors.database).catch(console.error);
				
				if (res.rows.length && duration) {
					message.client.database.query(`UPDATE trade_block SET expires_at = '${new Date(Date.now() + duration).toISOString()}'`)
					...
					message.client.database.query(`INSERT INTO trade_block (user_id, blocked_user_id${duration ? `, expires_at` : ""}) VALUES ('${message.author.id}', '${user.id}'${duration ? `, '${new Date(Date.now() + duration).toISOString()}'` : ""})`)
						.then(() => message.channel.send(language.get(language.blocked, user.tag, timeToString(duration / 1000, languageCode))).catch(console.error))
						.catch(err => {
							console.error(err);
							message.channel.send(language.errors.database).catch(console.error);
			...
			case "unblock":
				message.client.database.query(`DELETE FROM trade_block WHERE user_id = '${message.author.id}' AND blocked_user_id = '${user.id}'`)
			...
			case "start":
				const { "rows": blocked } = (await message.client.database.query(`SELECT * FROM trade_block WHERE user_id = '${user.id}' AND blocked_user_id = '${message.author.id}'`).catch(console.error)) || {};
				...
				collector.on("end", async () => {
					...
					await message.client.database.query("SELECT pokedex_id FROM pokemons WHERE pokedex_id = 0").catch(console.error);
		...
		async function checkValidPokemons(user1, pokemons1, user2, pokemons2) {
			const { "rows": user1Pokemons } = (await message.client.database.query(`SELECT * FROM pokemons WHERE users ? '${user1.id}'`).catch(console.error)) || {};
			const { "rows": user2Pokemons } = (await message.client.database.query(`SELECT * FROM pokemons WHERE users ? '${user2.id}'`).catch(console.error)) || {};
