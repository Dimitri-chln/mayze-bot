...
const command = {
	...
	run: async (message, args, options, language, languageCode) => {
		...
		if (!pokemonName) {
			const { rows } = (await message.client.database.query(`SELECT * FROM pokemon_hunting WHERE user_id = '${message.author.id}'`).catch(console.error)) || {};
		...
		if (pokemonName === "none") {
			message.client.database.query(`DELETE FROM pokemon_hunting WHERE user_id = '${message.author.id}'`)
				.then(() => {
		...
		if (collected.first().emoji.name === "✅") {
			await message.client.database.query(`INSERT INTO pokemon_hunting VALUES ('${message.author.id}', ${pokemon.national_id}) ON CONFLICT (user_id) DO UPDATE SET pokemon_id = ${pokemon.national_id}, hunt_count = 0 WHERE pokemon_hunting.user_id = '${message.author.id}'`);
