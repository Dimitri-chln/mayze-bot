...
const command = {
	...
	*/
	run: async (message, args, options, language, languageCode) => {
		...
		let { "rows": chatTop } = (await message.client.database.query("SELECT * FROM levels ORDER BY chat_xp DESC").catch(console.err)) || {};
		let { "rows": voiceTop } = (await message.client.database.query("SELECT * FROM levels ORDER BY voice_xp DESC").catch(console.err)) || {};
