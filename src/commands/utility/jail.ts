...
const command: Command = {
	...
	run: async (interaction: CommandInteraction, languageStrings: LanguageStrings) => {
		...
		if (!member.roles.cache.has(jailedRole.id)) {
			const unJailedRoles = member.roles.cache.filter(role => interaction.guild.roles.cache.some(r => r.name === role.name + " (Jailed)"));
			const jailedRoles = interaction.guild.roles.cache.filter(role => member.roles.cache.some(r => role.name === r.name + " (Jailed)"));
			...
			const jailedRoles = member.roles.cache.filter(role => interaction.guild.roles.cache.some(r => role.name === r.name + " (Jailed)"));
			const unJailedRoles = interaction.guild.roles.cache.filter(role => member.roles.cache.some(r => r.name === role.name + " (Jailed)"));
			jailedRoles.set(jailedRole.id, jailedRole);
