...
const command: Command = {
	...
	run: async (interaction, translations) => {
			...
			embeds: [
				{
					...
					thumbnail: {
						url: `https://dummyimage.com/100/${RGBToHex(
							color,
						).replace("#", "")}/00.png?text=+`,
					},
					footer: {
		...
		Object.values(emojis).forEach(
			async (e) => await reply.react(e).catch(console.error),
		);
