...
const command: Command = {
	...
	run: async (interaction, translations) => {
		...
		const type = REGEX_LIST.YOUTUBE_VIDEO.test(url)
			? "video"
			: REGEX_LIST.YOUTUBE_PLAYLIST.test(url)
			? "playlist"
			: "unknown";
		...
		switch (type) {
			case "video": {
				...
				embed
					.setDescription(
						MusicUtil.buildBar(result.duration, result.duration),
					)
					.setFooter({
			...
			case "playlist": {
				...
				for (const video of playlistInfo.videos)
					await downloadVideo(
						...
						playlistInfo.videos.indexOf(video),
				...
				embed
					.setDescription(
						MusicUtil.buildBar(
							playlistInfo.videos.slice(-1)[0].duration * 1000,
							playlistInfo.videos.slice(-1)[0].duration * 1000,
						),
					)
					.setFooter({
