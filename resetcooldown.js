module.exports = {
    ...
    execute(message, args) {
        ...
        if (message.author.id === ownerID) {
            ...
            if (!command) return message.channel.send(`Il n'y a pas de commandes ayant pour nom ou alias \`${commandName}\`, ${message.author}!`);
            const timestamps = message.client.cooldowns.get(command.name);
            if (timestamps.has(userID)) {
                timestamps.delete(userID);
            };
            message.channel.send(`Le cooldown de la commande \`${command.name}\` a été réinitialisé pour <@${userID}>`);
        } else {
