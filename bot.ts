...
dotenv.config();
import languages = require("./utils/languages");
import prototypes = require("./utils/prototypes");
import Discord = require("discord.js");
import { BetterClient } from "./utils/better-discord";
import shell = require("./utils/shellExec");
import pg = require("pg");
import { Command, ComponentCommand, ReactionCommand, ResponseCommand } from "./typings/interfaces";
import { version } from "./package.json";

const intents = new Discord.Intents([ Discord.Intents.NON_PRIVILEGED, "GUILD_MEMBERS", "GUILD_PRESENCES" ]);
const client = new BetterClient({ presence: { activity: { name: "le meilleur clan", type: "WATCHING" } }, fetchAllMembers: true, partials: ["MESSAGE", "CHANNEL", "REACTION"] , ws: { intents }});

if (process.env.HOST !== "HEROKU" && !process.env.NO_DATABASE_FETCH) {
	const output = shell.execute("heroku pg:credentials:url --app mayze-bot");
...
for (const file of musicCommandFiles) {
	const command: Command = require(`./music_commands/${file}`);
...
for (const file of responseFiles) {
	const response: ResponseCommand = require(`./responses/${file}`);
...
for (const file of reactionCommandsFiles) {
	const reactionCommand: ReactionCommand = require(`./reaction_commands/${file}`);
...
for (const file of componentCommandsFiles) {
	const componentCommand: ComponentCommand = require(`./components/${file}`);
