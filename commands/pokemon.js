			...
			case "list": {
				...
				for (i = 0; i < pokemons.length; i += pkmPerPage) {
					embed = new MessageEmbed()
						...
						.setColor(message.guild.me.displayColor)
						.setDescription(pokemons.slice(i, i + pkmPerPage).map(p => language.get(language.description, getPokemonName(pokedex.findPokemon(p.pokedex_id), p.shiny, p.variation, languageCode, "badge"), getPokemonName(pokedex.findPokemon(p.pokedex_id), p.shiny, p.variation, languageCode, "raw"), hasParam(params, "id") === 0 ? `#${p.pokedex_id}` : "", p.users[user.id].nickname, p.users[user.id].caught, p.users[user.id].caught > 1, p.users[user.id].favorite ? `https~d//pokemon.com/${languageCode === "en" ? "us" : languageCode}/pokedex/${pokedex.findPokemon(p.pokedex_id).names[languageCode].toLowerCase().replace(/[:\.']/g, "").replace(/\s/g, "-").replace(/\u2642/, "-male").replace(/\u2640/, "-female")}` : "")).join("\n"));
