	...
	execute: async (message, args, options, language, languageCode) => {
		...
		switch (subCommand) {
			case "add": {
				...
				else message.reply(language.response_added, { ephemeral: true }).catch(console.error);
				break;
			}
			case "remove": {
				const n = args
				...
				if (!res) return message.channel.send(language.errors.database, { ephemeral: true }).catch(console.error);
				if (!message.isInteraction) message.react("✅").catch(console.error);
				else message.reply(language.response_removed, { ephemeral: true }).catch(console.error);
				break;
			}
			case "get":
