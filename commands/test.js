...
const command = {
	...
	async execute(message, args) {
		...
		async function getPokemons(data) {
			...
			const count = backup.filter(p => p.pokedex_id === pkm.pokedex_id && p.is_shiny === pkm.is_shiny).length;
			const newData = data.filter(p => p.pokedex_id !== pkm.pokedex_id || p.is_shiny !== pkm.is_shiny);
