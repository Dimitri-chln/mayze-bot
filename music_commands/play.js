...
const command = {
	...
	execute: async (message, args, options) => {
        ...
        if (isPlaying) {
            ...
            let song = await message.client.player.addToQueue(message.guild.id, args.join(" "));
            song = song.song;
            message.channel.send(`<a:blackCheck:803603780666523699> | **Ajouté à la queue**\n> ${song.name}`).catch(console.error);
        } else {
            // Else, play the song
            let song = await message.client.player.play(message.member.voice.channel, args.join(" "));
            song = song.song;
