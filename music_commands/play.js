...
const command = {
	...
	execute: async (message, args, options) => {
        ...
        if (isPlaying) {
            ...
            const { song } = await message.client.player.addToQueue(message.guild.id, args.join(" "), null, message.author);
            message.channel.send(`<a:blackCheck:803603780666523699> | **Ajouté à la queue**\n> ${song.name}`).catch(console.error);
        } else {
            // Else, play the song
            const { song } = await message.client.player.play(message.member.voice.channel, args.join(" "), null, message.author);
